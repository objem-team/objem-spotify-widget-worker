...
pub async fn get_auth_token_from_cookie(req:&Request,ctx:&RouteContext<()>)->std::result::Result<Token,worker::Result<Response>>{
    let cookie_string = match req.headers().get("cookie") {
        Ok(cookie) => cookie,
        Err(err) => return Err(Response::error(format!("UNAUTHORIZED : can't get cookie header \n {:?}",err), StatusCode::UNAUTHORIZED.as_u16())),
    };
    let cookie_string = match cookie_string {
        Some(cookie) => cookie,
        None => return Err(Response::error("UNAUTHORIZED : cookie is None ", StatusCode::UNAUTHORIZED.as_u16())),
    };
    let cookie = get_cookie_from_string(cookie_string);
    let session_id = match cookie.get("session_id"){
        Some(session_id) => session_id,
        None => return Err(Response::error("UNAUTHORIZED", StatusCode::UNAUTHORIZED.as_u16())),
    };

    let kv = match ctx.kv("SESSION_KV"){
        Ok(kv) => kv,
        Err(err) => return Err(Response::error(format!("INTERNAL_SERVER_ERROR : Can't get KvStore \n {:?}",err), StatusCode::INTERNAL_SERVER_ERROR.as_u16())),
    };

    let token_json = match kv.get(session_id).await {
        Ok(token_json) => token_json,
        Err(err) => return Err(Response::error(format!("UNAUTHORIZED : can't get session value from KvStore \n {:?}",err), StatusCode::UNAUTHORIZED.as_u16())),
    };

    let token_json = match token_json {
        Some(token_json) => token_json.as_string(),
        None => return Err(Response::error("UNAUTHORIZED : there is no session", StatusCode::UNAUTHORIZED.as_u16())),
    };
...
pub async fn get_auth_code_spotify(req:&Request,ctx:&RouteContext<()>)->std::result::Result<AuthCodeSpotify,worker::Result<Response>>{
