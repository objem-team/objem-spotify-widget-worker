...
pub async fn handler(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    ...
    let token = match spotify.get_token().lock().await {
        ...
        Err(err) => {
            return Response::error(
                format!("INTERNAL_SERVER_ERROR : Can't lock memory \n {:?}", err),
                StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
            )
        }
    };
    ...
    let token = match token {
        ...
        None => {
            return Response::error(
                "INTERNAL_SERVER_ERROR : can't get token",
                StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
            )
        }
    };
    ...
    let token_json = match serde_json::to_string(&token) {
        ...
        Err(err) => {
            return Response::error(
                format!("INTERNAL_SERVER_ERROR : Can't parse Json \n {:?}", err),
                StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
            )
        }
    };
    ...
    response.headers_mut().append(
        "Set-Cookie",
        format!(
            "session_id={}; Max-Age={}; Secure; SameSite=None;",
            uuid, session_ttl
        )
        .as_str(),
    )?;
